...
***** 空头周期，寻找底部钝化和结构
	...
	Begin
      ...
      #+BEGIN_SRC vbs
         ...
         // 2. 保持“钝化”状态
         If dhStatus[i+1] = CI_LINFENG_DUNHUA Then
            If diff0[i] < diff0[ dMinIndex[ _cMaxIndexCounter - CI_LASTPERIOD ] ] Then
               Begin
                   dhStatus[i] := CI_LINFENG_DUNHUA;
                   If diff0[i] > diff0[ _diffTurnPoint ] Then    //diff值在上升，如果下跌转折超过阈值，则脱离钝化状态，无需判断
                       _diffTurnPoint := i;
         ...
         If dhStatus[i+1] = CI_GEFENG_DUNHUA Then
            If diff0[i] < diff0[ dMinIndex[ _cMaxIndexCounter - CI_LLASTPERIOD ] ] Then
            Begin
                dhStatus[i] := CI_GEFENG_DUNHUA;
                If diff0[i] > diff0[ _diffTurnPoint ] Then   //diff值在上升
                    _diffTurnPoint := i;
        ...
        // 3. 从“钝化”进入“钝化消失”
        If ( dhStatus[i+1] =CI_LINFENG_DUNHUA And
             diff0[i] > diff0[ dMinIndex[ _cMaxIndexCounter - CI_LASTPERIOD ]]) OR
           ( dhStatus[i+1] =CI_GEFENG_DUNHUA And
             diff0[i] > diff0[ dMinIndex[ _cMaxIndexCounter - CI_LLASTPERIOD ]]) THEN
                    dhStatus[i] := CI_DUNHUA_DISAPPEAR;
