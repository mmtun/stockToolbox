...
***** 多头周期，寻找顶部钝化和结构，启动循环
      ...
      #+BEGIN_SRC 
	_cMaxIndexCounter := 1;
        //当多头、空头周期多余2个时才考虑进行周期的判断（避免数组越界访问）
	...
	Begin
      ...
      #+BEGIN_SRC 
          IF macd0[i] > 0 THEN    //多头周期，判断顶部钝化和结构，循环开始。
          BEGIN
       #+END_SRC
	 ...
	 当结构形成以后，若DIFF值重新掉头向上时，则判断出现“结构消失”。 当结构消失后，后续重新进入“无状态”的阶段，判断是否再次出现“钝化”。
      ...
      #+BEGIN_SRC 
              ...
              If  (dhStatus[i+1] = CI_UNSET Or dhStatus[i+1] = CI_JIEGOU_DISAPPEAR) And
                  macd0[i+1] > 0 Then
              Begin
                  //临峰钝化判断
             ...
             // 4. 从“钝化”进入“结构形成”
             If ( dhStatus[i+1] = CI_LINFENG_DUNHUA OR dhStatus[i+1] = CI_GEFENG_DUNHUA ) Then
             Begin
             ...
             If ( dhStatus[i+1] = CI_LINFENG_DB_DUNHUA OR dhStatus[i+1] = CI_GEFENG_DB_DUNHUA ) Then
             Begin
                 If diff0[i] > diff0[ _diffTurnPoint ] And
                    Abs( diff0[i] - diff0[ _diffTurnPoint ] )/ diff0[ _diffTurnPoint ] >= CI_DIFF_TURN_THRESHOLD Then
                    Begin
...
***** 处理空头周期，寻找底部钝化和结构
      ...
      #+BEGIN_SRC
        //开始循环，处理空头周期的K线数据。
        _cMinIndexCounter := 1;
        If cMinIndex[0] >= _cMinIndexCounter+1 Then
        Begin
            i := cMinIndex[ _cMinIndexCounter ];
            Repeat
            BEGIN
      #+END_SRC
      ...
      IF macd0[i] < 0 THEN    //空头周期，判断顶部钝化和结构，循环开始。
      BEGIN
         If macd0[i+1] >=0 Then //上一条K线的MACD取值为0或负数，则进入新的一个空头周期
             _cMinIndexCounter := _cMinIndexCounter + 1;
         // 1. 从“无状态”或“结构消失”状态进入“钝化”
         If  (dhStatus[i+1] = CI_UNSET Or dhStatus[i+1] = CI_DB_JIEGOU_DISAPPEAR) And
             macd0[i+1] < 0 Then
         Begin
             //临峰钝化判断
             If closeData[i] < closeData[ cMinIndex[ _cMinIndexCounter - CI_LASTPERIOD ] ] And
         ...
         // 2. 保持“钝化”状态
         If dhStatus[i+1] = CI_LINFENG_DB_DUNHUA Then
         ...
         If dhStatus[i+1] = CI_GEFENG_DB_DUNHUA Then
        ...
        // 3. 从“钝化”进入“钝化消失”
        If ( dhStatus[i+1] =CI_LINFENG_DB_DUNHUA And
             diff0[i] < diff0[ dMinIndex[ _cMinIndexCounter - CI_LASTPERIOD ]]) OR
           ( dhStatus[i+1] =CI_GEFENG_DB_DUNHUA And
             diff0[i] < diff0[ dMinIndex[ _cMinIndexCounter - CI_LLASTPERIOD ]]) THEN
