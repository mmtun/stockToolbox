...
*****  0. 程序头
      #+BEGIN_SRC vbs
        ...
        Begin
            ...
            If jiegouCount[i+1] >=0 Then
            Begin
               jiegouCount[i] := jiegouCount[i+1] + 1;

               If dhStatus[i] = CI_JIEGOU_DISAPPEAR then
                   jiegouCount[i] := -1;

               If jiegouCount[i] > 24 Then
                   jiegouCount[i] := -1;
            End

            If dbJiegouCount[i+1] >=0 Then
            Begin
               dbJiegouCount[i] := dbJiegouCount[i+1] + 1;

               If dhStatus[i] = CI_DB_JIEGOU_DISAPPEAR then
                   dbJiegouCount[i] := -1;

               If dbJiegouCount[i] > 24 Then
                   dbJiegouCount[i] := -1;
            End
...
*****  1. 从“无状态”或“结构消失”进入“钝化”
      #+BEGIN_SRC vbs
            If  dhStatus[i] = CI_UNSET OR dhStatus[i] = CI_JIEGOU_DISAPPEAR OR
                dhStatus[i] = CI_DUNHUA_DISAPPEAR OR
                dhStatus[i] = CI_DB_DUNHUA_DISAPPEAR Then
            Begin
                // 多头周期的顶部钝化判断
                If macd0[i] > 0 And macd0[i+1] > 0 Then
                Begin
...
**** 以绘图方式进行测试
     ...
     #+BEGIN_SRC vbs
       ...
       DrawText( dhStatus = CI_JIEGOU And jiegouCount = 0, H, '!'), Align0,Pxup13,LineThick24,colorYellow;
       //DrawNumber( jiegouCount > 0, H, jiegouCount, 0),Align0,Pxup4,LineThick12,colorYellow; //显示结构后的周期数
       ...
       DrawText( dhStatus = CI_DB_JIEGOU And dbJiegouCount = 0, L, '!'), Align0,Pxdn30,LineThick24,colorYellow;
       //DrawNumber( dbJiegouCount > 0, H, dbJiegouCount, 0),Align0,Pxdn4,LineThick12,colorYellow; //显示结构后的周期数
       DrawText( dhStatus = CI_DB_JIEGOU_DISAPPEAR, L, '(!)'), Align0,Pxdn30,LineThick24,colorYellow;

       JiegouGS : dbJiegouCount,NoDraw,ColorWhite;
     #+END_SRC
